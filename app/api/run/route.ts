...
import {
  claudeRequestCapacity,
  claudeRequestsPerMinute,
} from "@/lib/pipeline/claude";
import { isPipelineConfigured, pipelineConfigMessage } from "@/lib/pipeline/config";
import {
  estimateMaximumClaudeRequests,
  estimateMaximumEmbeddingRequests,
  runPipeline,
...
export async function POST(request: NextRequest) {
  if (!isPipelineConfigured()) {
    return NextResponse.json(
      {
        // Names whichever key is actually missing. The run needs both
        // providers, and "set GEMINI_API_KEY" sent someone looking in the
        // wrong place once the generative stages moved to Claude.
  ...
  // Checked per provider. The two are rate-limited separately, and since the
  // generative stages moved to Claude a single combined total would be
  // measured against whichever ceiling happened to be imported here — which
  // is how a batch gets admitted that cannot actually finish.
  const answerCount = validated.input.answers.length;
  const admission = [
    {
      requests: estimateMaximumClaudeRequests(answerCount),
      capacity: claudeRequestCapacity(RUN_BUDGET_MS),
      rpm: claudeRequestsPerMinute(),
      setting: "ANTHROPIC_RPM",
    },
    {
      requests: estimateMaximumEmbeddingRequests(answerCount),
      capacity: geminiRequestCapacity(RUN_BUDGET_MS),
      rpm: geminiRequestsPerMinute(),
      setting: "GEMINI_RPM",
    },
  ].find((provider) => provider.requests > provider.capacity);

  if (admission) {
    return NextResponse.json(
      {
        error: `This batch can require ${admission.requests} AI requests, but the configured ${admission.rpm} RPM limit can admit ${admission.capacity} within the run time budget. Use a smaller batch or raise ${admission.setting} only when your provider quota supports it.`,
