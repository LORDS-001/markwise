...
vi.mock("@/lib/pipeline/run", () => ({
  runPipeline,
  // Claude takes one call per answer plus one per two-person cluster;
  // Gemini batches every signature into a single embedding call.
  estimateMaximumClaudeRequests: (answerCount: number) =>
    answerCount + Math.floor(answerCount / 2),
  estimateMaximumEmbeddingRequests: (answerCount: number) =>
    Math.ceil(answerCount / 100),
}));
...
beforeEach(() => {
  ...
  delete process.env.ANTHROPIC_RPM;
...
describe("authorizing live work", () => {
  ...
  it("rejects a batch that cannot pass the configured limiter within the run budget", async () => {
    // Extraction and cluster assessment run on Claude, so it is that limit a
    // large batch runs into first. At 5 RPM the budget admits 25 starts and
    // 50 answers need 75.
    process.env.ANTHROPIC_RPM = "5";
    vi.resetModules();
    ...
    const payload = await response.json();
    expect(payload).toMatchObject({ code: "batch_exceeds_runtime" });
    // Naming the wrong setting sends someone to raise a limit that was never
    // the constraint.
    expect(payload.error).toContain("ANTHROPIC_RPM");
    expect(authorizeAiRequest).not.toHaveBeenCalled();
    expect(runPipeline).not.toHaveBeenCalled();
  });
});
...
describe("when the pipeline is not configured", () => {
  ...
  it("names the missing key rather than the one that happens to be checked first", async () => {
    // A run needs both providers. Reporting the Gemini key when the Anthropic
    // one is missing is how someone spends an afternoon on the wrong console.
    delete process.env.ANTHROPIC_API_KEY;
    vi.resetModules();
    ...
    expect(payload.error).not.toContain("GEMINI_API_KEY");
    expect(runPipeline).not.toHaveBeenCalled();
  });
});
